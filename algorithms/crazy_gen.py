...
class Monsters():
    def __init__(self, locations, edges, population_size=100,
                 ...
                 hybrid=False):
        """
        Generalization of the Genetic Algorithm to a continuum representation.
        =======================================================================================
        A state in the population is represented as a list of numbers [0,1]
    ...
    def _fast_mutate(self, individual, mutation_rate):
        ...
        # Switch any of the locations at random
        for i in range(number_of_mutations):
            [a, b] = np.random.choice(range(len(individual)),2,replace=False)
    ...
    def get_edges(self, individual):
        individual = np.array(self.compress_information(individual))
        return [self.index_edge[i] for i in np.where(individual == 1.)[0]]

    def compress_information(self,individual):
        winners = np.argsort(individual)[:-self.number_of_locations-1:-1]
        return [1 if i in winners else 0 for i in range(self.number_of_edges)]
    ...
    def _count_connected_sets(self,individual):
        ...
        """
        return 1 # This is because atm there is no way to implement this
    ...
    def _entropy(self,individual):
        norm = self.number_of_locations*np.log(self.number_of_locations/self.number_of_edges)
        return np.sum([x * np.log(x) for x in individual if x > 0])/norm
    ...
    def _rank_routes(self,population):
        ...
        entropy_cost = {i: self._entropy(route)
                        for i, route in enumerate(population)}
        ...
        fitness_results = {i: 1./cost[i] for i in pop_route_len.keys()}
    ...
    def _fast_breed(self, parent1, parent2):
        ...
        return r * parent1[0] + (1-r) * parent2[0]
